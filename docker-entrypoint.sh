...
[ "`id www-data -u`" = "${PUID}" ] || (usermod -u ${PUID} app; echo -n 'U';)
[ "`id www-data -g`" = "${PGID}" ] || (groupmod -g ${PGID} app; echo -n 'G';)
[ "`stat --format '%U%G' /var/www/html`" = www-datawww-data ] || (chown -R www-data:www-data /var/www/html; echo -n 'H';)
